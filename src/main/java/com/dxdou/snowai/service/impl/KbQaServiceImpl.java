...
public class KbQaServiceImpl extends ServiceImpl<KbChatHistoryMapper, KbChatHistory> implements KbQaService {
    ...
    public QaResponse chat(Long kbId, QaRequest request) {
        ...
        try {
            ...
            List<KbDocument> relevantDocs = searchService
                    .semanticSearch(request.getQuestion(), kbId, new Page<>(1, 5), null)
                    .getRecords()
                    .stream()
                    .map(vo -> {
            ...
            saveChatHistory(request.getSessionId(), kbId, request.getQuestion(), answer, model.getId(), processTime);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SseEmitter streamChat(Long kbId, QaRequest request) {
        ...
        try {
            ...
            streamCallAiModel(model, prompt, chunk -> {
                answer.append(chunk);
                try {
                    emitter.send(chunk);
                } catch (Exception e) {
            ...
            // 5. 保存对话历史
            long processTime = System.currentTimeMillis() - startTime;
            saveChatHistory(request.getSessionId(), kbId, request.getQuestion(), answer.toString(), model.getId(),
                    processTime);
     ...
     * @param callback 回调函数
     */
    private void streamCallAiModel(LlmConfig model, String prompt, java.util.function.Consumer<String> callback) {
        // 1. 构建请求头
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.set("Authorization", "Bearer " + model.getApiKey());
        ...
        webClient.post()
                ...
                .subscribe(
                        chunk -> {
                            try {
                                ...
                                if (chunk.startsWith("data: ")) {
                                    ...
                                    // 解析JSON数据
                                    Map<String, Object> response = parseJson(data);
                        ...
                        error -> {
                            throw new BusinessException("流式调用失败: " + error.getMessage());
                        });
    }
     ...
     */
    private void saveChatHistory(String sessionId, Long kbId, String question, String answer, Long modelId,
            long processTime) {
